        ...
        plugins: [
            ...
            new webpack.DefinePlugin({
                ...
                DEVMODE_WARN_AGAINST_CLIENTRENDERED_REGIONS: BUILD_ENV === 'production' ?
                    '' :
                    '\nregionPathsPostfilled.push(component.path);\nif (!regionsRemaining[regionName] && regionPathsPostfilled.length) {\n\tconsole.warn(`React4xp postfilled ${regionPathsPostfilled.length} component(s) (path(s): ${JSON.stringify(regionPathsPostfilled.join(", ")}).\n\nThis is an attempted fallback, compensating for when React4xp is client-side-rendering an XP page/layout that contains regions. This means the components in the regions need to be filled in by the server in a second rendering step.\n\nNOTE: Currently, this extra step can only fetch the HTML of the component. So if the component has pageContributions, these will be omitted! If possible, avoid using React4xp client-side-rendering for a region container (i.e. page/layout) where the region(s) has component(s) that need pageContributions to work.`);\n}'
            }),
        ],
    };
};
